...
export default async function RankingPage() {
  ...
  return (
    <div>
      ...
      {me && (
        <div className="card p-3 mb-3 flex items-center gap-3" style={{ borderColor: "var(--forest)" }}>
          <span className="font-bold w-8 text-center">{me.rank}</span>
          <span className="flex-1 font-semibold">{me.nickname} <span className="chip ml-1">나</span></span>
          <span className="text-sm">{me.summit_count}개 · {me.total_elevation.toLocaleString()}m</span>
        </div>
      )}
        ...
        <ol className="card divide-y divide-[color:var(--line)]">
          {list.map((r) => (
            <li key={r.user_id} className="flex items-center gap-3 p-3">
              <span className="w-8 text-center font-bold text-lg">{medal(r.rank)}</span>
              ...
              <div className="text-right">
                <p className="font-bold">{r.summit_count}<span className="text-xs muted font-normal">개</span></p>
                <p className="text-xs muted">{r.total_elevation.toLocaleString()}m</p>
              </div>
            </li>
          ))}
        </ol>
